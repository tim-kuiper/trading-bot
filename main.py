...
while True:
  ...
  if len(list_24h) == 1:
    ...
    list_24h.clear()
  if len(list_168h) == 7 or len(start_list_168h) == 1:
    timeframe = "1w"
    file_extension = '.json'
    asset_file = timeframe + file_extension 
    asset_file_path = './' + asset_file
    interval_time_minutes = 10080
    interval_time_simple = '1w'
    order_size = 1200
    sll_trigger_pct = 1.14 # trigger pct from current price
    sll_limit_pct = 1.15 # limit pct from current price
    for asset_pair in asset_pairs:
      api_key = get_asset_vars()[2]
      api_sec = get_asset_vars()[1]
      leverage = get_asset_vars()[3]
      ...
      time.sleep(3) # sleep 3 seconds between asset pair
    list_168h.clear()
  if len(list_360h) == 15 or len(start_list_360h) == 1:
    timeframe = "15d"
    file_extension = '.json'
    asset_file = timeframe + file_extension 
    asset_file_path = './' + asset_file
    interval_time_minutes = 21600
    interval_time_simple = '15d'
    order_size = 1800
    sll_trigger_pct = 1.24 # trigger pct from current price
    sll_limit_pct = 1.25 # limit pct from current price
    for asset_pair in asset_pairs:
      api_key = get_asset_vars()[2]
      api_sec = get_asset_vars()[1]
      leverage = get_asset_vars()[3]
